...
#include <thread> // this thread :: sleep_for
#include <chrono>
...
/*bool input_password(string temp_password ,string secured_password){
    cout << "Password : ";
    char ch = getch();
    int i;
    bool verified = true;
    while(ch != '\r'){
        cout << "*";
        if (ch != secured_password[i]) {verified = false;}
        i++;
        ch = getch();
    }
    return verified;
...
struct wanted
{
    ...
    int x;
    int selected_flights[NumofFlights + 1] = {};
}wanted1 = {};
struct tickets
{
...
void display_selected_flights(int y[]) {
    ...
    cout << endl;
    int numOFselectedflight = wanted1.selected_flights[NumofFlights + 1];
...
above1:
    ...
    try_again:
        ...
        if (c > wanted1.selected_flights[NumofFlights + 1] || c <= 0) {
            ...
            cin >> choice;
            if (compareStrings(choice, "y") || compareStrings(choice, "Y")) { goto try_again; }
...
{
    bool flight_okay = true;
    int failed_flights = 0;
    cout << "\nEnter number of flights you want to add:" << endl;
    cin >> Entered_Flights;
    if (Entered_Flights > NumofFlights) { error("You entered more than maximum number of flights"); return ADMIN_DISPLAY; }
    for (int i = Start_Flights; i < Start_Flights + Entered_Flights; i++)
    {
        ...
        if (compareStrings(temp_f1.arrival, temp_f1.departure)) { error("You go no where :("); failed_flights++; flight_okay = false; }  //       
        if (flight_okay) { f1[i] = temp_f1; }
...
int deleteflight(flight f1[NumofFlights])
{
    ...
    bool loop = false;
    cin >> z;
    if (z <= 0) { Invalid(); return ADMIN_DISPLAY; }
    // z-1 is the number must be deleted 
    for (int i = z; i <= Start_Flights; i++)
    {
        loop = true;
        f1[i - 1] = f1[i];
    }
    if (!loop) { Invalid(); return ADMIN_DISPLAY; }
    Start_Flights--;
...
int updateflight(flight f1[NumofFlights])
{
    ...
    if (Updated_Flight > Start_Flights) { Invalid(); return ADMIN_DISPLAY; }
...
int modify_ticket(tickets tickets1[Full_Tickets]) {
    ...
    cin >> z;
    if (z <= 0) { Invalid(); return PASSENGER; }
    // z-1 is the number must be deleted 
    ...
    if (!loop) { Invalid(); return PASSENGER; }
    Num_of_ticket--;
...
int SignupDisplay() {
    ...
    bool new_id = true;
    cout << "\n\t    ==================================================\nSignup Menu:\n-----------\n";
    if (Accounts_Used == Full_Users) { cout << "\t\tSorry but the amount of users is full\n"; return USER; }
    else {
